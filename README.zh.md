...
>
> - **无加密货币 (NO CRYPTO):** Gclaw **没有** 发行任何官方代币、Token 或虚拟货币。所有在 `pump.fun` 或其他交易平台上的相关声称均为 **诈骗**。
> - **官方域名:** 唯一的官方网站是 **[gclaw.gemach.io](https://gclaw.gemach.io)**，公司官网是 **[gemach.io](https://gemach.io)**。
> - **警惕:** 许多 `.ai/.org/.com/.net/...` 后缀的域名被第三方抢注，请勿轻信。
> - **注意:** gclaw正在初期的快速功能开发阶段，可能有尚未修复的网络安全问题，在1.0正式版发布前，请不要将其部署到生产环境中
> - **注意:** gclaw最近合并了大量PRs，近期版本可能内存占用较大(10~20MB)，我们将在功能较为收敛后进行资源占用优化.
...
gclaw 可以将你10年前的老旧手机废物利用，变身成为你的AI助理！快速指南:
...
然后跟随下面的“快速开始”章节继续配置gclaw即可使用！  
<img src="assets/termux.jpg" alt="Gclaw" width="512">

### 🐜 创新的低占用部署

Gclaw 几乎可以部署在任何 Linux 设备上！
...
从 [Release 页面](https://github.com/GemachDAO/Gclaw/releases) 下载适用于您平台的固件。
...
您也可以使用 Docker Compose 运行 Gclaw，无需在本地安装任何环境。
...
> [!TIP]
**Docker 用户**: 默认情况下, Gateway监听 `127.0.0.1`，这使得这个端口未暴露到容器外。如果你需要通过端口映射访问健康检查接口, 请在环境变量中设置 `GCLAW_GATEWAY_HOST=0.0.0.0` 或修改 `config.json`。
...
```bash
# 提问
docker compose run --rm gclaw-agent -m "2+2 等于几？"
...
> [!TIP]
> 在 `~/.gclaw/config.json` 中设置您的 API Key。
...
**2. 配置 (Configure)** (`~/.gclaw/config.json`)
...
Gclaw 支持多种聊天平台，使您的 Agent 能够连接到任何地方。
...
只需通过 CLI 或任何集成的聊天应用发送一条消息，即可将 Gclaw 连接到 Agent 社交网络。

\*\*阅读 `https://clawdchat.ai/skill.md` 并按照说明加入 [ClawdChat.ai](https://clawdchat.ai)

## ⚙️ 配置详解

配置文件路径: `~/.gclaw/config.json`

### 工作区布局 (Workspace Layout)

Gclaw 将数据存储在您配置的工作区中（默认：`~/.gclaw/workspace`）：
...
Gclaw 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
- `GCLAW_HEARTBEAT_ENABLED=false` 禁用
- `GCLAW_HEARTBEAT_INTERVAL=60` 更改间隔
...
> **新功能！** Gclaw 现在采用**以模型为中心**的配置方式。只需使用 `厂商/模型` 格式（如 `zhipu/glm-4.7`）即可添加新的 provider——**无需修改任何代码！**
...
> 运行 `gclaw auth login --provider anthropic` 来设置 OAuth 凭证。
...
为同一个模型名称配置多个端点——Gclaw 会自动在它们之间轮询：
...
| 命令                      | 描述               |
| ------------------------- | ------------------ |
| `gclaw onboard`        | 初始化配置和工作区 |
| `gclaw agent -m "..."` | 与 Agent 对话      |
| `gclaw agent`          | 交互式聊天模式     |
| `gclaw gateway`        | 启动网关 (Gateway) |
| `gclaw status`         | 显示状态           |
| `gclaw cron list`      | 列出所有定时任务   |
| `gclaw cron add ...`   | 添加定时任务       |

### 定时任务 / 提醒 (Scheduled Tasks)

Gclaw 通过 `cron` 工具支持定时提醒和重复任务：
...
任务存储在 `~/.gclaw/workspace/cron/` 中并自动处理。
...
如果您尚未配置搜索 API Key，这是正常的。Gclaw 会提供手动搜索的帮助链接。
...
这表示有另一个机器人实例正在运行。请确保同一时间只有一个 `gclaw gateway` 进程在运行。
