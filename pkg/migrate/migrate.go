...
func Run(opts Options) (*Result, error) {
	...
	gclawHome, err := resolveGclawHome(opts.GclawHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, gclawHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to Gclaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, gclawHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, gclawHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(gclawHome, "config.json"),
				Description: "convert OpenClaw config to Gclaw format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(gclawHome)
...
func Execute(actions []Action, openclawHome, gclawHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, gclawHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, gclawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing Gclaw config: %w", err)
		}
...
func resolveGclawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("GCLAW_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".gclaw"), nil
}
