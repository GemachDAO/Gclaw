...
func authHelp() {
	...
	fmt.Println("  gclaw auth login --provider openai")
	fmt.Println("  gclaw auth login --provider openai --device-code")
	fmt.Println("  gclaw auth login --provider anthropic")
	fmt.Println("  gclaw auth login --provider google-antigravity")
	fmt.Println("  gclaw auth models")
	fmt.Println("  gclaw auth logout --provider openai")
	fmt.Println("  gclaw auth status")
}
...
func authLoginGoogleAntigravity() {
	...
	fmt.Println("Try it: gclaw agent -m \"Hello world\"")
}
...
func authStatusCmd() {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: gclaw auth login --provider <name>")
		return
	}
...
func authModelsCmd() {
	...
	if err != nil || cred == nil {
		...
		fmt.Println("Run: gclaw auth login --provider google-antigravity")
		return
	}
