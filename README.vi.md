...
<h1>Gclaw: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>
...
>
> * **KHÔNG CÓ CRYPTO:** Gclaw **KHÔNG** có bất kỳ token/coin chính thức nào. Mọi thông tin trên `pump.fun` hoặc các sàn giao dịch khác đều là **LỪA ĐẢO**.
> * **DOMAIN CHÍNH THỨC:** Website chính thức **DUY NHẤT** là **[gclaw.gemach.io](https://gclaw.gemach.io)**, website công ty là **[gemach.io](https://gemach.io)**.
> * **Cảnh báo:** Nhiều tên miền `.ai/.org/.com/.net/...` đã bị bên thứ ba đăng ký, không phải của chúng tôi.
> * **Cảnh báo:** Gclaw đang trong giai đoạn phát triển sớm và có thể còn các vấn đề bảo mật mạng chưa được giải quyết. Không nên triển khai lên môi trường production trước phiên bản v1.0.
> * **Lưu ý:** Gclaw gần đây đã merge nhiều PR, dẫn đến bộ nhớ sử dụng có thể lớn hơn (10–20MB) ở các phiên bản mới nhất. Chúng tôi sẽ ưu tiên tối ưu tài nguyên khi bộ tính năng đã ổn định.
...
Gclaw có thể triển khai trên hầu hết mọi thiết bị Linux!
...
Tải file binary cho nền tảng của bạn từ [trang Release](https://github.com/GemachDAO/Gclaw/releases).
...
Bạn cũng có thể chạy Gclaw bằng Docker Compose mà không cần cài đặt gì trên máy.
...
> [!TIP]
> **Người dùng Docker**: Theo mặc định, Gateway lắng nghe trên `127.0.0.1`, không thể truy cập từ máy chủ. Nếu bạn cần truy cập các endpoint kiểm tra sức khỏe hoặc mở cổng, hãy đặt `GCLAW_GATEWAY_HOST=0.0.0.0` trong môi trường của bạn hoặc cập nhật `config.json`.
...
```bash
# Đặt câu hỏi
docker compose run --rm gclaw-agent -m "2+2 bằng mấy?"
...
> [!TIP]
> Thiết lập API key trong `~/.gclaw/config.json`.
...
```bash
gclaw agent -m "Xin chào, bạn là ai?"
```
...
Trò chuyện với Gclaw qua Telegram, Discord, DingTalk, LINE hoặc WeCom.
...
> **Docker Compose**: Thêm `ports: ["18791:18791"]` vào service `gclaw-gateway` để mở port webhook.
...
Gclaw hỗ trợ hai loại tích hợp WeCom:
...
Kết nối Gclaw với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn qua CLI hoặc bất kỳ ứng dụng Chat nào đã tích hợp.

**Đọc `https://clawdchat.ai/skill.md` và làm theo hướng dẫn để tham gia [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Cấu hình chi tiết

File cấu hình: `~/.gclaw/config.json`

### Cấu trúc Workspace

Gclaw lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.gclaw/workspace`):
...
Gclaw chạy trong môi trường sandbox theo mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong phạm vi workspace.
...
|----------|---------|-------|
| `workspace` | `~/.gclaw/workspace` | Thư mục làm việc của agent |
...
Gclaw có thể tự động thực hiện các tác vụ định kỳ. Tạo file `HEARTBEAT.md` trong workspace:
...
* `GCLAW_HEARTBEAT_ENABLED=false` để tắt
* `GCLAW_HEARTBEAT_INTERVAL=60` để thay đổi khoảng thời gian
...
> **Tính năng mới!** Gclaw hiện sử dụng phương pháp cấu hình **đặt mô hình vào trung tâm**. Chỉ cần chỉ định dạng `nhà cung cấp/mô hình` (ví dụ: `zhipu/glm-4.7`) để thêm nhà cung cấp mới—**không cần thay đổi mã!**
...
```
> Chạy `gclaw auth login --provider anthropic` để thiết lập thông tin xác thực OAuth.

#### Cân bằng Tải tải

Định cấu hình nhiều endpoint cho cùng một tên mô hình—Gclaw sẽ tự động phân phối round-robin giữa chúng:
...
| Lệnh | Mô tả |
| --- | --- |
| `gclaw onboard` | Khởi tạo cấu hình & workspace |
| `gclaw agent -m "..."` | Trò chuyện với agent |
| `gclaw agent` | Chế độ chat tương tác |
| `gclaw gateway` | Khởi động gateway (cho bot chat) |
| `gclaw status` | Hiển thị trạng thái |
| `gclaw cron list` | Liệt kê tất cả tác vụ định kỳ |
| `gclaw cron add ...` | Thêm tác vụ định kỳ |

### Tác vụ định kỳ / Nhắc nhở

Gclaw hỗ trợ nhắc nhở theo lịch và tác vụ lặp lại thông qua công cụ `cron`:
...
Các tác vụ được lưu trong `~/.gclaw/workspace/cron/` và được xử lý tự động.
...
Điều này là bình thường nếu bạn chưa cấu hình API key cho tìm kiếm. Gclaw sẽ cung cấp các liên kết hữu ích để tìm kiếm thủ công.
...
Thêm key vào `~/.gclaw/config.json` nếu dùng Brave:
...
Điều này xảy ra khi có một instance bot khác đang chạy. Đảm bảo chỉ có một tiến trình `gclaw gateway` chạy tại một thời điểm.
