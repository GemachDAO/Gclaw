...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Gclaw.
...
10. [Creating a New Provider in Gclaw](#creating-a-new-provider-in-gclaw)
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<GclawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- Gclaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.gclaw/auth.json`:
...
## Creating a New Provider in Gclaw

Gclaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/gclaw/cmd_auth.go`:
...
gclaw auth login --provider your-provider
...
gclaw agent -m "Hello" --model your-model
```
...
export GCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export GCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `gclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.gclaw/auth.json`
- Re-run `gclaw auth login --provider antigravity`
