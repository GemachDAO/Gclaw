...
<h1>Gclaw: Go で書かれた超効率 AI アシスタント</h1>
...
### 🐜 革新的な省フットプリントデプロイ
Gclaw はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/GemachDAO/Gclaw/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに Gclaw を実行できます。
...
> [!TIP]
> **Docker ユーザー**: デフォルトでは、Gateway は `127.0.0.1` でリッスンしており、ホストからアクセスできません。ヘルスチェックエンドポイントにアクセスしたり、ポートを公開したりする必要がある場合は、環境変数で `GCLAW_GATEWAY_HOST=0.0.0.0` を設定するか、`config.json` を更新してください。
...
> [!TIP]
> `~/.gclaw/config.json` に API キーを設定してください。
...
Telegram、Discord、QQ、DingTalk、LINE、WeCom で Gclaw と会話できます
...
> **Docker Compose**: `gclaw-gateway` サービスに `ports: ["18791:18791"]` を追加して Webhook ポートを公開してください。
...
Gclaw は2種類の WeCom 統合をサポートしています：
...
設定ファイル: `~/.gclaw/config.json`

### ワークスペース構成

Gclaw は設定されたワークスペース（デフォルト: `~/.gclaw/workspace`）にデータを保存します：
...
Gclaw はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.gclaw/workspace` | エージェントの作業ディレクトリ |
...
Gclaw は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `GCLAW_HEARTBEAT_ENABLED=false` で無効化
- `GCLAW_HEARTBEAT_INTERVAL=60` で間隔変更
...
> **新機能！** Gclaw は現在 **モデル中心** の設定アプローチを採用しています。`ベンダー/モデル` 形式（例: `zhipu/glm-4.7`）を指定するだけで、新しいプロバイダーを追加できます—**コードの変更は一切不要！**
...
```
> OAuth認証を設定するには、`gclaw auth login --provider anthropic` を実行してください。

#### ロードバランシング

同じモデル名で複数のエンドポイントを設定すると、Gclaw が自動的にラウンドロビンで分散します：
...
| コマンド | 説明 |
|---------|------|
| `gclaw onboard` | 設定＆ワークスペースの初期化 |
| `gclaw agent -m "..."` | エージェントとチャット |
| `gclaw agent` | インタラクティブチャットモード |
| `gclaw gateway` | ゲートウェイを起動 |
| `gclaw status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。Gclaw は手動検索用の便利なリンクを提供します。
...
別のインスタンスが実行中の場合に発生します。`gclaw gateway` が 1 つだけ実行されていることを確認してください。
