...
def test_long_take_profit_leg_pays_tp_minus_fee():
    ...
    r = forge.trade_return(candles, 0, is_long=True, stop_pct=2.0)
    assert r == 0.03 - forge.TAKER_COST


def test_long_stop_leg_pays_minus_stop_minus_fee():
    candles = _bars(100.0, {"h": 101.0, "l": 97.0, "c": 98.0})  # tags the 98 stop
    r = forge.trade_return(candles, 0, is_long=True, stop_pct=2.0)
    assert r == -0.02 - forge.TAKER_COST
...
def test_short_take_profit_leg():
    ...
    r = forge.trade_return(candles, 0, is_long=False, stop_pct=2.0)
    assert r == 0.03 - forge.TAKER_COST
...
def test_no_leg_hit_marks_out_at_last_close():
    ...
    r = forge.trade_return(candles, 0, is_long=True, stop_pct=2.0)
    assert r == (100.5 / 100.0 - 1) - forge.TAKER_COST
...
def test_walk_is_capped_at_max_hold():
    ...
    r = forge.trade_return(flat, 0, is_long=True, stop_pct=2.0)
    assert isinstance(r, float)


def test_fee_is_realistic_and_floor_is_above_it():
    ...
    assert forge.TAKER_COST >= 0.0009
    assert forge.PROVE_MIN_EDGE >= forge.TAKER_COST
